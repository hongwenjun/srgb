...
// 2013.10.23  添加打开目的文件夹功能; 解决原图和新图同一个目录，删除文件的BUG ; 拼版使用了图片很多次，只复制一次
// 源码地址:  https://code.google.com/p/srgb/source/browse/adobe_js/AI打包链接图片.js 
...
if (app.documents.length != 0) {
    myFolder= Folder.selectDialog ("AICS链接图收集打包脚本--汉化增强版:蘭公子2008-2013 \n现在开始收集当前AI文档的链接图,请指定你要保存链接图的文件夹", DiyFolder);
    ExportLinks();  //收集链接图应用函数
	
// ConvertText();        // 开启文字转曲功能: 把本行最前面 "//" 删除就可以

    EndInfo = "AICS链接图收集打包脚本--汉化增强版:蘭公子2008-2013\n\nAI文档的链接图已经收集到指定的目录\n\nHttp://sRGB.googlecode.com  sRGB色彩联盟, QQ群:2471313\n用RGB颜色空间矫色，使你的图片色彩更丰富！\n\n①(批量打包链接图,先关闭所有的AI文件执行本脚本)\n②(文字转曲开启，请修改本脚本第18行)\n"
...
// 批量打包AI文件的链接图,请先关闭所有的AI文件
    ...
    myScanFolder= Folder.selectDialog ("AICS链接图收集打包脚本--汉化增强版:蘭公子2008-2013n①来源：选择Adobe Illustrator文件(*.ai)的文件夹",DiyFolder);
...
function ExportLinks() {
	for (i=0;i<app.activeDocument.placedItems.length;i++) {
        ...
        if (newFile.exists ){
        // 解决原图和新图同一个目录，删除文件的BUG ; 拼版使用了图片很多次，只复制一次
        } else
		linkedFile.copy(newFile);
//屏蔽更新图片		app.activeDocument.placedItems[i].file = newFile;

//~     Debuginfo = linkedFile.absoluteURI  + "\n" +newFile.absoluteURI ;
//~     alert(Debuginfo);   // 调试信息
